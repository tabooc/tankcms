...
} elseif($_POST['act']=="开始测试")//网速测试，待完善。
	{
?>
<script>
var acd1;
...
?>
<div class="bar">
  <div class="barli" style="width:<?=$percent?>%">&nbsp;</div>
</div>
<?php
...
<head>
<meta charset="utf-8" />
<title>PHP探针 - <?php echo $version; ?></title>
<style type="text/css">
...
<script src="<?php echo STATIC_JSPATH.'jquery-1.9.1.min.js'?>"></script>
<script>
...
<div id="page">
  ...
  <table width="100%" cellpadding="3" cellspacing="0" align="center">
    <tr>
      <th colspan="6">服务器CPU及内存相关运行参数</th>
    </tr>
    <tr>
      <td>CPU核数</td>
...
?>
        物理内存：共 <font color='#CC0000'><?php echo $memTotal;?> </font> , 已用 <font color='#CC0000'><span id="UsedMemory">0</span></font> , 空闲 <font color='#CC0000'><span id="FreeMemory">0</span></font> , 使用率 <span id="memPercent">0</span>
        <div class="bar">
          <div id="barmemPercent" class="barli_green" >&nbsp;</div>
        </div>
        Cache化内存为 <span id="CachedMemory">0</span> , 真实内存使用率为 <span id="memRealPercent">0</span> %
        <div class="bar">
          <div id="barmemRealPercent" class="barli_blue" >&nbsp;</div>
        </div>
        SWAP区：共 <?php echo $st;?> , 已使用 <span id="swapUsed">0</span> , 空闲 <span id="swapFree">0</span> , 使用率 <span id="swapPercent">0</span> %
        <div class="bar">
...
?>
  ...
  <table width="100%" cellpadding="3" cellspacing="0" align="center">
    ...
    <tr>
      ...
      <td width="15%"><?php
		...
		?>
        <?php echo (false!==eregi("phpinfo",$disFuns))? 'NO' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?></td>
    ...
    <tr>
      <td colspan="4">被禁用的函数（disable_functions）：
        <?=(""==($disFuns=get_cfg_var("disable_functions")))?"无":str_replace(",",", ",$disFuns)?></td>
    </tr>
    <tr>
      <td colspan="4">默认支持函数（enable_functions）：
        <?=(false!==eregi("function",$disFuns))?NO:"<a href='$phpSelf?act=Function' target='_blank' class='static'>请点这里查看详细！</a>"?></td>
    </tr>
  </table>
  ...
  <form action="<?php echo $_SERVER[PHP_SELF]."#bottom";?>" method="post">
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
      ...
      <tr align="center">
        <td>本台服务器</td>
        <td><?php echo $valInt;?><br />
          <input class="btn" name="act" type="submit" value="整型测试" /></td>
        <td><?php echo $valFloat;?><br />
          <input class="btn" name="act" type="submit" value="浮点测试" /></td>
        <td><?php echo $valIo;?><br />
          <input class="btn" name="act" type="submit" value="IO测试" /></td>
        <td></td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
      ...
      <tr>
        <td width="15%"></td>
        <td width="60%"> 地址：
          <input type="text" name="host" value="localhost" size="10" />
          端口：
          <input type="text" name="port" value="3306" size="10" />
          用户名：
          <input type="text" name="login" size="10" />
          密码：
          <input type="password" name="password" size="10" /></td>
        <td width="25%"><input class="btn" type="submit" name="act" value="MySQL检测" /></td>
      </tr>
    </table>
    <?php
  ...
  if ($_POST['act'] == 'MySQL检测') {
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
      ...
      <tr>
        <td width="15%"></td>
        <td width="60%"> 请输入您要检测的函数：
          <input type="text" name="funName" size="50" /></td>
        <td width="25%"><input class="btn" type="submit" name="act" align="right" value="函数检测" /></td>
      </tr>
      <?php
  ...
  ?>
    ...
    <table width="100%" cellpadding="3" cellspacing="0" align="center">
      ...
      <tr>
        <td width="15%"></td>
        <td width="60%"> 请输入您要检测的邮件地址：
          <input type="text" name="mailAdd" size="50" /></td>
        <td width="25%"><input class="btn" type="submit" name="act" value="邮件检测" /></td>
      </tr>
      <?php
  ...
  <a id="bottom"></a>
  <div id="footer"> &copy; <?php echo date("Y",time());?> PHP探针
